...
void loop() {
  ...
  if((val1 == LOW) && (val2 == LOW)){
    ...
    } //buttonPin1 detects "healthy food" so LEDPin2 is "light"
  else if((val2 == LOW) && (val1 == HIGH)){
    Spark.publish("pikachu_status", "Pikachu is SAD because you are eating junk food", 60);
    Particle.publish("pika2", "430053001151353338363333"); //communicate with Yuchen's photon
    delay(5000); 
...
void feed(const char *event, const char *data) {
    Spark.publish("pikachu_feed", "Good Morning, Pikachu needs to EAT!", 60);
}
